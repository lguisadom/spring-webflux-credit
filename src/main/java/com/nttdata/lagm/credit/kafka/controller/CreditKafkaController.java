package com.nttdata.lagm.credit.kafka.controller;
...
import com.nttdata.lagm.credit.kafka.service.CreditServiceKafka;
...
@RestController
@RequestMapping("/api/kafka/v1/credit")
public class CreditKafkaController {
	
	private Logger LOGGER = LoggerFactory.getLogger(CreditKafkaController.class);
	
	@Autowired
	private CreditService creditService;
	
	@Autowired
	private CreditServiceKafka creditServiceKafka;
	...
	private Mono<Credit> create(@RequestBody CreditRequestDto creditRequestDto) {
		return creditServiceKafka.create(creditRequestDto);
	}
